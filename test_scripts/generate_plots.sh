...
protocols=("dummy-sctp" "stop-and-wait")
logfiles=("sctp.log" "snw.log")

for (( i=0; i<${#protocols[@]}; i++ )); do
        # SNW delay grows linearly
        # ../test_scripts/run.sh -n "${num_pkts}" -d "100ms" -w 0 -l "random 0%" -t 10 -i 100 -p "${protocols[$i]}" -f "${logfiles[$i]}"
